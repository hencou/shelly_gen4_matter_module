...
/* A bus stuck low reads nine 0x00 bytes, and nine 0x00 bytes have CRC8 0 — so
 * the CRC check passes and the caller reports a perfectly plausible 0.00 C. A
 * bus stuck high reads all 0xFF the same way. Reject both. */
static bool ow_scratchpad_degenerate(const uint8_t *sc)
{
    int zeros = 0, ones = 0;
    for (int i = 0; i < 9; i++) {
        if (sc[i] == 0x00) zeros++;
...
static bool ds18b20_read_centi_c(int16_t *out)
{
    ...
    if (ow_scratchpad_degenerate(sc)) {
        s_temp_err = "bus stuck (all-zero/all-ff scratchpad), no sensor answering";
        return false;
    }
