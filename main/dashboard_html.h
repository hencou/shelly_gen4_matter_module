...
"restart.</p>"
"<p class=info><b>Enable WiFi 10 min</b> joins WiFi (station only) next to a "
"running Thread network, without a reboot, and switches WiFi off again after "
"10 minutes. WiFi and Thread share one radio, so for the duration the device "
"gives up its Thread router role and its SRP fallback server; both come back "
"automatically. Pressing again extends the window, or closes it early while "
"it is open.</p>"
...
"<button class='btn btn-blue' onclick=doWifiCoex()>Enable WiFi 10 min</button>"
...
"</td></tr>"
"<tr><td>Temporary WiFi<br><span style='font-size:.8em;color:#666'>"
"WiFi station next to Thread for 10 minutes, no reboot. Thread router role "
"and SRP fallback stand down for the duration.</span></td>"
"<td class=hw-val id=hw-wtemp>-<br>"
"<button class='btn btn-gray' onclick=doWifiCoex() style='margin-top:.4em'>Toggle</button>"
"</td></tr>"
...
/* Temporary WiFi next to Thread (no reboot) */
"function doWifiCoex(){"
"  var x=new XMLHttpRequest();"
"  x.onload=function(){document.getElementById('wifi-msg').innerHTML="
"    (x.status==200?'<span class=ok>':'<span class=err>')+x.responseText+'</span>'};"
"  x.onerror=function(){document.getElementById('wifi-msg').innerHTML="
"    '<span class=err>Connection error</span>'};"
"  x.open('POST','/api/wifi-coex');x.send();"
"}"
